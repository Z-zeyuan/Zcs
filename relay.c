...
char *ip2A = "224.1.2.1";
char *ip2S = "224.1.2.2";
char *Lan1Relay_ip = "224.1.1.10";
char *Lan2Relay_ip = "224.1.2.10";



mcast_t *Lan1_FROM_App_TO_Service_Receive;
mcast_t *Lan1_FROM_Service_TO_App_Receive;
mcast_t *Lan2_FROM_App_TO_Service_Receive;
mcast_t *Lan2_FROM_Service_TO_App_Receive;
...
void *Lan1AppListener(){
    multicast_setup_recv(Lan1_FROM_Service_TO_App_Receive);
    char msg[MAX_MSG_LENGTH];
    char *msgTS = (char *)malloc(MAX_MSG_LENGTH * sizeof(char));

    while(join_threads == 0){
        //printf("ServiceListener is alive...\n");
        memset(msg,'\0',sizeof(msg));
        while(multicast_check_receive(Lan1_FROM_Service_TO_App_Receive) == 0){
            //printf("Service is waiting for message...\n");
        }
        multicast_receive(Lan1_FROM_Service_TO_App_Receive,msg,MAX_MSG_LENGTH);
        printf("Lan1App's new msg: %s\n",msg);
        memset(msgTS,'\0',sizeof(msgTS));
        strcpy(msgTS,msg);
        multicast_send(Lan2_Service_App_Send,msgTS,MAX_MSG_LENGTH);
        printf("Msg to Lan2's A: %s\n",msgTS);

    }
}

void *Lan1ServiceListener(){
    multicast_setup_recv(Lan1_FROM_App_TO_Service_Receive);
    char msg[MAX_MSG_LENGTH];
    char *msgTS = (char *)malloc(MAX_MSG_LENGTH * sizeof(char));
    ...
    while(join_threads == 0){
        //printf("ServiceListener is alive...\n");
        memset(msg,'\0',sizeof(msg));
        while(multicast_check_receive(Lan1_FROM_App_TO_Service_Receive) == 0){
            //printf("Service is waiting for message...\n");
        }
        multicast_receive(Lan1_FROM_App_TO_Service_Receive,msg,MAX_MSG_LENGTH);
        printf("Lan1Service's new msg: %s\n",msg);
        memset(msgTS,'\0',sizeof(msgTS));
        strcpy(msgTS,msg);
        multicast_send(Lan2_App_Service_Send,msgTS,strlen(msgTS));
        printf("Msg to Lan2's S: %s\n",msgTS);
    }
}

void *Lan2AppListener(){
    multicast_setup_recv(Lan2_FROM_Service_TO_App_Receive);
    char msg[MAX_MSG_LENGTH];
    char *msgTS = (char *)malloc(MAX_MSG_LENGTH * sizeof(char));
    ...
    while(join_threads == 0){
        //printf("ServiceListener is alive...\n");
        memset(msg,'\0',sizeof(msg));
        while(multicast_check_receive(Lan2_FROM_Service_TO_App_Receive) == 0){
            //printf("Service is waiting for message...\n");
        }
        multicast_receive(Lan2_FROM_Service_TO_App_Receive,msg,MAX_MSG_LENGTH);
        printf("Lan2App's new msg: %s\n",msg);
        memset(msgTS,'\0',sizeof(msgTS));
        strcpy(msgTS,msg);
        multicast_send(Lan1_Service_App_Send,msgTS,MAX_MSG_LENGTH);
        printf("Msg to Lan1's A: %s\n",msgTS);
    }
}

void *Lan2ServiceListener(){
    multicast_setup_recv(Lan2_FROM_App_TO_Service_Receive);
    char msg[MAX_MSG_LENGTH];
    char *msgTS = (char *)malloc(MAX_MSG_LENGTH * sizeof(char));
    ...
    while(join_threads == 0){
        //printf("ServiceListener is alive...\n");
        memset(msg,'\0',sizeof(msg));
        while(multicast_check_receive(Lan2_FROM_App_TO_Service_Receive) == 0){
            //printf("Service is waiting for message...\n");
        }
        multicast_receive(Lan2_FROM_App_TO_Service_Receive,msg,MAX_MSG_LENGTH);
        printf("Lan1Service's new msg: %s\n",msg);
        memset(msgTS,'\0',sizeof(msgTS));
        strcpy(msgTS,msg);
        multicast_send(Lan1_App_Service_Send,msgTS,strlen(msgTS));
        printf("Msg to Lan1's S: %s\n",msgTS);
    }
}
...
int relay_init(){
    //Lan 1 channel
    Lan1_FROM_App_TO_Service_Receive = multicast_init(Lan1Relay_ip,DEFAULT_PORT,RELAY_TOSERVICE_PORT);
    Lan1_App_Service_Send = multicast_init(ip1A,APP_SEND_PORT,DEFAULT_PORT);
    Lan1_FROM_Service_TO_App_Receive = multicast_init(Lan1Relay_ip,DEFAULT_PORT,RELAY_TOAPP_PORT);
    Lan1_Service_App_Send = multicast_init(ip1S,SERVICE_SEND_PORT,DEFAULT_PORT);

    //Lan 2 channel
    Lan2_FROM_App_TO_Service_Receive = multicast_init(Lan2Relay_ip,DEFAULT_PORT,RELAY_TOSERVICE_PORT);
    Lan2_App_Service_Send = multicast_init(ip2A,APP_SEND_PORT,DEFAULT_PORT);
    Lan2_FROM_Service_TO_App_Receive = multicast_init(Lan2Relay_ip,DEFAULT_PORT,RELAY_TOAPP_PORT);
...
int relay_up(){
    ...
    //     pthread_create(&ListenerThread,NULL,AppListener,NULL);
    // }
    send_msg(Lan1_Service_App_Send,discovery,strlen(discovery),1);
    send_msg(Lan2_Service_App_Send,discovery,strlen(discovery),1);
